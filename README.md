...
suMMetry is a small 3x3 logic puzzle game built with vanilla HTML, CSS, and JavaScript. Fill the board so every row, every column, and both diagonals add up to the same target sum.
...
5. Complete the grid so all eight lines equal the target sum.
...
- Every row, every column, and both diagonals must equal the same target sum `S`.
